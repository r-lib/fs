describe("file_copy", {
  ...
  it("copies multiple files", {
    with_dir_tree(list("foo" = "test", "bar" = "test2"), {
      ...
      expect_equal(
        file_copy(c("foo", "bar"), c("foo2", "bar2")),
        fs_path(c("foo2", "bar2"))
      )
      expect_equal(readLines("foo2"), readLines("foo"))
...
describe("dir_copy", {
  ...
  it("can overwrite directories", {
    with_dir_tree(
      list("foo/bar" = "test", "foo2/bar" = "test2"),
      {
  ...
  it("copies absolute paths", {
    with_dir_tree(
      ...
      {
        expect_equal(
          dir_copy(path_abs("foo"), path_abs("foo2")),
          path_abs("foo2")
        )
        expect_true(dir_exists("foo2"))
  ...
  it("dir_copy() works for >1 path", {
    grandparent_before <- withr::local_tempdir(
      "dir_copy-multiple_paths-before-"
    )
