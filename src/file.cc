...
List stat_(CharacterVector path) {
  ...
  List out = List(17);
  CharacterVector names = CharacterVector(17);
  ...
  Rf_classgets(VECTOR_ELT(out, 2), Rf_mkString("octmode"));

  SET_STRING_ELT(names, 3, Rf_mkChar("hard_links"));
  SET_VECTOR_ELT(out, 3, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 4, Rf_mkChar("user_id"));
  SET_VECTOR_ELT(out, 4, Rf_allocVector(INTSXP, n));

  SET_STRING_ELT(names, 5, Rf_mkChar("group_id"));
  SET_VECTOR_ELT(out, 5, Rf_allocVector(INTSXP, n));

  SET_STRING_ELT(names, 6, Rf_mkChar("special_device_id"));
  SET_VECTOR_ELT(out, 6, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 7, Rf_mkChar("inode"));
  SET_VECTOR_ELT(out, 7, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 8, Rf_mkChar("size"));
  SET_VECTOR_ELT(out, 8, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 9, Rf_mkChar("block_size"));
  SET_VECTOR_ELT(out, 9, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 10, Rf_mkChar("blocks"));
  SET_VECTOR_ELT(out, 10, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 11, Rf_mkChar("flags"));
  SET_VECTOR_ELT(out, 11, Rf_allocVector(INTSXP, n));

  SET_STRING_ELT(names, 12, Rf_mkChar("generation"));
  SET_VECTOR_ELT(out, 12, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 13, Rf_mkChar("access_time"));
  SET_VECTOR_ELT(out, 13, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 14, Rf_mkChar("modification_time"));
  SET_VECTOR_ELT(out, 14, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 15, Rf_mkChar("creation_time"));
  SET_VECTOR_ELT(out, 15, Rf_allocVector(REALSXP, n));

  SET_STRING_ELT(names, 16, Rf_mkChar("birth_time"));
  ...
  for (size_t i = 0; i < Rf_xlength(path); ++i) {
    ...
    INTEGER(VECTOR_ELT(out, 2))[i] = st.st_mode;
    REAL(VECTOR_ELT(out, 3))[i] = st.st_nlink;
    INTEGER(VECTOR_ELT(out, 4))[i] = st.st_uid;
    INTEGER(VECTOR_ELT(out, 5))[i] = st.st_gid;
    REAL(VECTOR_ELT(out, 6))[i] = st.st_rdev;
    REAL(VECTOR_ELT(out, 7))[i] = st.st_ino;
    REAL(VECTOR_ELT(out, 8))[i] = st.st_size;
    REAL(VECTOR_ELT(out, 9))[i] = st.st_blksize;
    REAL(VECTOR_ELT(out, 10))[i] = st.st_blocks;
    INTEGER(VECTOR_ELT(out, 11))[i] = st.st_flags;
    REAL(VECTOR_ELT(out, 12))[i] = st.st_gen;

    REAL(VECTOR_ELT(out, 13))
